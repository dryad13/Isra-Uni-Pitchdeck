...
# NOTE: Do not call screeninfo.get_monitors() at import time. On headless/server
# hosts (the OMR API and batch read pipeline) the native backend can hard-abort
# the process (SIGABRT), which try/except cannot catch. Display windows are only
# used by InteractionUtils.show() in interactive debugging, so we default the
# window size and resolve the real monitor lazily, guarded, only when shown.
...
class ImageMetrics:
    ...
    window_width, window_height = _DEFAULT_WINDOW_SIZE
...
class InteractionUtils:
    ...
    def show(name, origin, pause=1, resize=False, reset_pos=None, config=None):
        ...
        image_metrics.window_width, image_metrics.window_height = _resolve_window_size()
        if origin is None:
